...
// it has complete HashMap implementation internally works using Hashing technique. 
public class LRUCacheImplement {
	
public static void main(String[] args) throws java.lang.Exception {
		
		LRUCacheImplement lrucache = new LRUCacheImplement();
	...
	public void putEntry(int key, int value) {
		if (hashmap.containsKey(key)) // Key Already Exist, just update the value and move it to top
		{
			...
			newnode.value = value;
			newnode.key = key;
			if (hashmap.size() > LRU_SIZE) // We have reached maxium siz, so need to make bucket for new element.
			{
